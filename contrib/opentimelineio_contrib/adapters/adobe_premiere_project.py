...
class AdobePremiereProject(object):
    ...
    def _get_object(self, id_, attr_name="ObjectID"):
        if id_ not in self._object_cache:
            ...
            if not node:
                raise AdobePremiereProjectParseError("Couldn't find object with {} {}".format(attr_name, id_))
    ...
    def _dereference(self, node):
        ...
        if not id_:
            raise AdobePremiereProjectParseError("Node has no ObjectRef attribute: {}".format(node))
        return self._get_object(id_)
    
    def _udereference(self, node):
        uid = node.get("ObjectURef")
        if not uid:
            raise AdobePremiereProjectParseError("Node has no ObjectURef attribute: {}".format(node))
        ...
        for track_group_node in self._dereference_all(
            ...
            for track_node in self._dereference_all(
                ...
                if track_node.tag == "VideoClipTrack":
                    ...
                    raise AdobePremiereProjectParseError("Unknown track type: {} (ObjectID: {})".format(track_node.tag, track_node.get("ObjectID")))
                ...
                for top_track_item_node in self._dereference_all(
                    ...
                    if track_start > last_track_end:
                        track.append(
                            otio.schema.Gap(
                                duration=otio.opentime.RationalTime(track_start - last_track_end)
                            )
                        )
                    ...
                    clip_in = otio.opentime.RationalTime(int(clip_node.find("InPoint").text) / 254016000000)
                    clip_out = otio.opentime.RationalTime(int(clip_node.find("OutPoint").text) / 254016000000)
                    ...
                    if media_node_ref is not None:
                        ...
                        sequence_node_ref = media_source_node.find("SequenceSource/Sequence")
